$(document).ready(function() {
	setTimeout(function(){
		...
		$('#entri_kegiatan').parsley().on('field:validated', function() {
		...
		$('.buttonFinish').on("click", function(){
			...
			var level = $('.x_panel').find('.x_content #level').val();
			...
			var target = $('.x_panel').find('.x_content #target').val();
			var realisasi = $('.x_panel').find('.x_content #realisasi').val();
			...
			if(!bidang){
				...
				$('.info').addClass('hidden');
				$('.x_panel').find("#entri_kegiatan").submit();
			}else if(!kategori){
				...
				$('.x_panel').find('.x_content #level').prop('required',true);
				$(".peringatan").find("#keterangan").text("Data Kegiatan - Level kegiatan belum dipilih.");
				$('.info').addClass('hidden');
				$('.x_panel').find("#entri_kegiatan").submit();
			}else if(!deskripsi){
				...
				$('.x_panel').find('.x_content #target').prop('required',true);
				$(".peringatan").find("#keterangan").text("Jumlah Peserta - Target belum diisi.");
				$('.info').addClass('hidden');
				$('.x_panel').find("#entri_kegiatan").submit();
			}else if(!realisasi){
				$('.x_panel').find('.x_content #realisasi').prop('required',true);
				$(".peringatan").find("#keterangan").text("Jumlah Peserta - Realisasi belum diisi.");
				$('.info').addClass('hidden');
				$('.x_panel').find("#entri_kegiatan").submit();
			}else if(!foto){
				$('.x_panel').find('.x_content #foto_upload').prop('required',true);
				$(".peringatan").find("#keterangan").text("Foto Kegiatan - Foto kegiatan belum ada.");
				$('.info').addClass('hidden');
				$('.x_panel').find("#entri_kegiatan").submit();
			}else{
				var op = $("<input>").attr("type", "hidden").attr("name", "op").val("tambah");
				$(".x_panel").find("#entri_kegiatan").append($(op));
				$('.x_panel').find("#entri_kegiatan").submit();
			}
		});
	},10);
...
/* tidak dipakai karena kegiatan input bukan dropdown
	...
	$('.x_panel').find('.x_content #kegiatan').on("change", function(){
		$('.peringatan').addClass('hidden');
	});

	$('.x_panel').find('.x_content #level').on("change", function(){
		$('.peringatan').addClass('hidden');
	});
	...
	setTimeout(function(){
		...
		$.get("/misc/kegiatan/kecamatan", {dpd}, function(data,status){
			...
			$.each(arrayData, function(index, value){
				var arrayOpt = value.split('_');
				if(arrayOpt[0] == kecamatan){
					selected = 'selected';
					$(".x_panel").find('.x_content #kecamatan').append("<option id='opt_kecamatan' value='"+arrayOpt[0]+"' selected>"+arrayOpt[1]+"</option>");
				}else{
					$(".x_panel").find('.x_content #kecamatan').append("<option id='opt_kecamatan' value='"+arrayOpt[0]+"'>"+arrayOpt[1]+"</option>");
				}
			});
		}).fail(function(){
			alert("data kecamatan tidak tersedia");
		});
	},20);
